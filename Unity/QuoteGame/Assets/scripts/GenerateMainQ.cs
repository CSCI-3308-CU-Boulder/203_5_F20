...
public class GenerateMainQ : MonoBehaviour {
	...
	void Update()
	{
		...
		if (a.GetComponent<TimerCount>().timeRemaining <= 0)
		{
			...
			else if (w == 2)
			{
				stopDis();
				nextQuestion();
				a.GetComponent<TimerCount>().resetTimer();
				w = 0;
			}
	...
	public void readTextFileA(){
		...
		AnswerText = arlistA[globalIndex];
	...
	{
		serverConnection.GetComponent<ServerConnect>().endQuestion();
	}
	public void ShowAnswer()
	{
		ans.SetActive(true);
		Coption.GetComponent<Text>().text = "Correct Answer : " + AnswerText;
	}
	public void stopDis()
	{
		ans.SetActive(false);
